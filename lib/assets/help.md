...
- `Show Solution` - will display the solution to the current puzzle. If you have guessed Cells in a wrong way, mark the cell and display the wrong and correct value.You may use `Clear Hints` then to remove the solution.
...
- `Show Tips` - will display a list of tips for solving the current Sudoku puzzle. If the puzzle is not solvable anymore the grid is painted in red. If it 
  is solvable, it will show the alternatives for each cell.
